...
def create_train_state(key:jnp.array, QuAttnNet, num_modes:int, num_layers:int, learning_rate:float):
    ...
    params = QuAttnNet.init_params(layer_dims,key,num_modes, num_layers)['params']
...
def get_predictions_and_encoder_outputs(QuAttnNet,
    ...
    predictions = []
    true_labels = []
    QuAttnNet = PreSymbolicRegressionNet(num_modes=FLAGS.modes, num_layers=FLAGS.num_encode_layers)
    for i in range(len(train_ds['images'])):
