...
def save_encoder_outputs_and_predictions(QuAttnNet, params, train_ds, current_ds, regularization, suppress_k):
    if FLAGS.dense_decoder:
        ...
        doc.update({"DENSE_DECODER": FLAGS.dense_decoder, "DECODER_LAYER_DIMS": FLAGS.layer_dims})
        ...
        FILENAME = f"/InputForSymbolicRegressionRGZN{regularization}_M{FLAGS.shots}_epochs{best_epoch}"\
        ...
        with h5py.File(SAVING_PATH + FILENAME, "w") as f:
            ...
            f.create_dataset("best_epoch", data=best_epoch)
...
def save_predictions(params,classical_accuracy, nonclassical_accuracy, current_ds, regularization, suppress_k):
    doc = {"SHOTS": FLAGS.shots, "MINIBATCH_SIZE": FLAGS.batch_size,
        "MODES": FLAGS.modes, "LEARNING_RATE": FLAGS.learning_rate,
        "NUM_ENCODE_LAYERS": FLAGS.num_encode_layers,
        "REGULARIZATION": regularization, "SUPPRESSING_K": suppress_k}
...
def main_training(rng,opt,state,params,QuAttnNet, epochs, train_ds, test_ds,regularization, suppress_k):
    ...
    return params, state, jnp.array(cl_accracy), jnp.array(ncl_accracy)
...
if __name__ == "__main__":
    ...
    params, state, cl_accracy, ncl_accracy = main_training(rng, 
                                                        opt, 
                                                        state, 
                                                        params, 
                                                        QuAttnNet,
    ...
    #save_predictions(params,cl_accracy, ncl_accracy, Current_dataset,regularization, suppress_k)