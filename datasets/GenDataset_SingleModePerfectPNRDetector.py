...
def Sampling_1mode(state_species:str, initial_state: np.array, shots:int, cutoff:int) -> np.array:
...
def Sampling_MixedCoherent(initial_states: list, shots:int, cutoff:int) -> np.array:
   measurements = []
...
def gen_ds(name_dataset:str, shots:int, cutoff:int) -> dict:
   img_classical = []
   img_non_classical = []
   fails = 0
   fail_configs = []
...
if __name__ == "__main__":
   ...
   s = Sampling_1mode('pats', [0.2], shots=100000, cutoff=29)
   fig, ax = plt.subplots()
   #print(s)
