...
def load_POVM(detector_type, abs_path):
...
def ClickDistr(POVM, sample_from_unitary):
    samples = []
...
def Sample_from_random_unitary_6modes(circuit, state_species, initial_state, shots, cutoff):
    '''         
...
def Sample_mixed_coherent_from_random_unitary_6modes(circuit, alpha0, alpha1, shots, cutoff):
...
def Sample_lossy_fock_from_random_unitary_6modes(circuit, n0, shots, cutoff):
...
def Sampling_with_POVM(abs_path,circuit, state_species, initial_state, shots, cutoff):
...
if __name__ == '__main__':
    ...
    samples2 = Sample_lossy_fock_from_random_unitary_6modes(decomposed_circuit,[0,0,0,2,2,1], KSHOTS*1000, 9)
    end_total = time.time()
